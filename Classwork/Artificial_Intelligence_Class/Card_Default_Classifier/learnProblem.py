            ...
            if one_hot and ftype == "categorical" and i != self.target_index:
                for val in frange:
                    def feat(e,index=i,val=val):
                        return e[index]==val
                    if self.header:
                        #Had to change this to avoid an error
                        feat.__doc__ = f"{self.header[i]}={val}"
                    else:
...
def themode(prediction):
    ...
    This handles all of the cases of AIPython predictors: dictionaries, lists and boolean probabilities.
    """
